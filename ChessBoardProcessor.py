...
class ChessBoardProcessor(object):
	...
	def __init__(self, blank_board=None):
		...
		Args:
		    blank_board (None, optional): Description
		"""
	...
	def perspective_transform(self, board_file):
		...
		ret, corners = cv2.findChessboardCorners(board_img,(7,7), flags=cv2.CALIB_CB_NORMALIZE_IMAGE|cv2.CALIB_CB_ADAPTIVE_THRESH)
		...
		current_corners = np.float32([[x_predict[0],y_predict[0]],[x_predict[1],y_predict[1]],[x_predict[2],y_predict[2]],[x_predict[3],y_predict[3]]]) 
		transform_corners = np.float32([[1080,1080],[1080,0],[0,0],[0,1080]])
	...
	def board_split(self, board_file, state=0):
		...
		Args:
		    board_image (path): Path to image to be split
		    state (int): 0 - data collection for training, 1 - game mode 
		# """
		...
		for i in range(8):
			for j in range(8):
				...
				if state == 1:
					...
					name = directory +  str(time.time()) + '_' + str(count) + '.jpg'
	...
	def label_image(self, piece_img):
		...
		while(1):
			piece = input("What type of piece (e - empty, p - pawn, n - night, b - bishop, r - rook, q - queen, k - king: ")
